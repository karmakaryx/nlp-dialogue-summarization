...
def clean_text(text: str) -> str:
    ...
    text = re.sub(r"(Mr\.|Mrs\.|Ms\.)([a-zA-Z])", r"\1 \2", text)
    text = re.sub(r"\n+", "\n", text)
    return text.strip()
...
def main(config):
    ...
    solar_df = preprocessor.make_set_as_df(os.path.join(DATA_PATH, "dev_solar.csv"), is_train=True)

    total_df = pd.concat([train_df, val_df, solar_df], axis=0).reset_index(drop=True)

    kf = GroupKFold(n_splits=config["training"]["n_splits"])

    exp_name = file_name.removeprefix("nlp_ds_")
    now = datetime.now().strftime("%m%d-%H%M")
    best_model_paths = []

    for fold, (t_idx, v_idx) in enumerate(kf.split(total_df, groups=total_df["fname"])):
...
#     best_paths = [
#         os.path.join(OUTPUT_PATH, "fold_1", "checkpoint-3456"),
#         os.path.join(OUTPUT_PATH, "fold_2", "checkpoint-2592"),
#         os.path.join(OUTPUT_PATH, "fold_3", "checkpoint-3024"),
#         os.path.join(OUTPUT_PATH, "fold_4", "checkpoint-3240"),
#         os.path.join(OUTPUT_PATH, "fold_5", "checkpoint-1728"),
#     ]
