...
config['general']['output_dir'] = EXP_PATH
config['wandb']['entity'] = TEAM
config['wandb']['dir'] = os.path.join(EXP_PATH, 'logs')
...
class Preprocess:
    ...
    def clean_dialogue(self, text: str) -> str:
        text = text.replace('\\n', '\n')
        text = re.sub(r'#?\s*Person\s*([1-7])\s*#?\s*[:\s]*', r'#Person\1#: ', text)
...
class SummaryDataset(Dataset):
    ...
    @staticmethod
    def sanitize_summary(dialogue, summary):
        tags = re.findall(r'#Person\d+#', dialogue)
        tags = list(set(tags))

        clean_summary = summary
        titles = ['Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Miss']
        for title in titles:
            pattern = rf'{re.escape(title)}\s*([A-Z][a-z]+)'
            names = re.findall(pattern, clean_summary)
            for name in names:
                clean_summary = clean_summary.replace(f'{title} {name}', '#Person1#')
                clean_summary = clean_summary.replace(name, '#Person1#')

        return clean_summary

    def __getitem__(self, idx):
        item = self.df.iloc[idx]
        cleaned_summary = self.sanitize_summary(item['dialogue'], item['summary'])

        instruction = '대화를 요약하세요. 인물의 이름 대신 반드시 #Person1#, #Person2# 태그만 사용하세요.'
        dialogue_part = f"### User:\n{instruction}\n\n### 대화:\n{item['dialogue']}\n\n### Assistant:\n"
        summary_part = f'{cleaned_summary}{self.tokenizer.eos_token}'
        ...
        summary_ids = self.tokenizer.encode(summary_part, add_special_tokens=False)
        prompt_len = len(input_ids) - len(summary_ids)
