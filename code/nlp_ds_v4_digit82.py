...
wandb.init(
    ...
    dir=os.path.join(EXP_PATH, 'logs'),
)
pprint(config)
...
class Preprocess:
    def __init__(self, bos_token: str, eos_token: str) -> None:
        ...
        self.instruction = '다음 대화를 요약하세요. 대화의 핵심 내용을 포함하고, 인물은 반드시 #Person1#, #Person2# 등의 태그로 지칭하세요.\n\n'
...
def inference(config):
    ...
    with torch.no_grad():
        for item in tqdm(dataloader):
            ...
            for text in decoded_batch:
                text = text.replace('<s>', '').replace('</s>', '').replace('<pad>', '').replace('<usr>', '')
    ...
    submission_df.to_csv(os.path.join(config['general']['output_dir'], 'output.csv'), index=True, index_label='', header=True)
